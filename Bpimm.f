...
cs
  ...
  270    f(i,i)=f(i,i)-qq(isort(j)+1)*qsig(j)*rrij
      DO 280 I=2,NA
         i1=isort(i)+1
      DO 280 J=1,I-1
cs
        rrij=rr(i,j)
        i2=isort(j)+1
        F(I,I)=F(I,I)-(CS(J)+qq(i2)*qsig(j))*RRIJ
        F(J,J)=F(J,J)-(CS(I)+qq(i1)*qsig(i))*RRIJ
        F(I,J)=H(I,J)-(0.5*P(I,J)-qq(i1)*qq(i2)*qsig(i)*qsig(j))*RRIJ
  280 F(J,I)=F(I,J)
cp
      if (nm.lt.-5) then
      do 281 i=1,na
...
C
  ...
  432       IF (VS(I,M).GT.0..AND.ISORT(M).EQ.0) IB=IB+1
      ...
      DO 431 J=I,NAA
         ...
         ISO2=ISORT(J)+1
         EBSIG=(DE(ISO1,ISO2)+DF(ISO1,ISO2)*
     1   (qsig(i)+cs(i))*(qsig(j)+cs(j))*rr(i,j))
...
cs
         rrii=0.25*eps*rxy(i,i)*cs(i)
         EPIB=EPIB+(0.25*P(I,I)*P(I,I)+0.5*(qq(isp1)*QSIG(I)*QQ(ISP1))
     1   *CS(I))*RR(I,I)-(hv(iso1)+rrii)*p(i,i)+h(i,i)*zv(iso1)
     2   -(ZV(ISO1)-1.)*rr(i,i)
         DO 440 J=1,I-1
         isp2=isort(j)+1
         rrij=rr(i,j)
         EPIB=EPIB+2.*P(I,J)*(H(I,J)+qq(isp1)*qq(isp2)*QSIG(I)*QSIG(J)
...
cs
     ...
     1 rvdw(50),ion(50)
cs
      DIMENSION qsa(ix),xneg(50),xeta(50),xdel(50),qsz(ix,ix),
     1qsza(ix,ix)
c     parameter
      do 1 i=1,50
      xneg(i)=10.
...
c          dqjl=0.
 ...
 23   continue
          ISO1=ISORT(I)+1
          iso2=isort(j)+1
          xeli=xneg(iso1)-dqik*xdel(iso1)
          xelj=xneg(iso2)-dqjl*xdel(iso2)
          dqij=abs((xeli-xelj)/(xeta(iso1)+xeta(iso2)))
