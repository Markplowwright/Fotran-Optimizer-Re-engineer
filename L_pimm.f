...
C     ( IX=201 --> IX=VALUE ) BEFORE COMPILATION OF THE PROGRAM.
C
C
      CHARACTER*2 ATA,ATN
      ...
      PARAMETER(IX=201,IY=IX,IX4=IX*4,IX6=IX*6)
      COMMON /LABEL/ ATA(50)
     ...
     1RESTY(IX),RESTYP(IX)
      common /elnegs/ xneg(ix,ix),xeta(ix,ix)
cp
...
C
  ...
  432       IF (VS(I,M).GT.0..AND.ISORT(M).EQ.0) IB=IB+1
      ...
      DO 431 J=I,NAA
         ...
         ISO2=ISORT(J)+1
        EBSIG=DE(ISO1,ISO2)+DF(ISO1,ISO2)*(qsig(i)+cs(i))*
     1  (qsig(j)+cs(j))*rr(i,j)
c         EBSIG=(DE(ISO1,ISO2)+DF(ISO1,ISO2)*(QSIG(I)*QSIG(J)+
c     1   QSIG(I)*CS(J)+CS(I)*QSIG(J))*RR(I,J))
c     2   *(1.-(1.-EXP(AME(ISO1,ISO2)*(AL(ISO1,ISO2)-VV(I,J))))**2)
         SUMSIG=SUMSIG+EBSIG
  431 CONTINUE
  430 CONTINUE
C
...
cs
         rrii=0.25*eps*rxy(i,i)*cs(i)
         EPIB=EPIB+(0.25*P(I,I)*P(I,I)+0.5*(QSIG(I)-QQ(ISP1))
     1   *CS(I))*RR(I,I)-(hv(iso1)+rrii)*p(i,i)+h(i,i)*zv(iso1)
     2   -(ZV(ISO1)-1.)*rr(i,i)
         DO 440 J=1,I-1
         isp2=isort(j)+1
         rrij=rr(i,j)
         EPIB=EPIB+2.*P(I,J)*(H(I,J)+QSIG(I)*QSIG(J)
     1   *RRIJ)-(0.5*P(I,J)*P(I,J)-CS(I)*CS(J))*RRIJ
  440    CONTINUE
  450 CONTINUE
C
...
C
     ...
     2115.,140.,91.,232.,59.,56.,59.,52.,12.,4*0.,11.,9*0./
      DATA XN/7.98,8.79,11.54,12.87,15.18,15.07,10.14,11.,14.66,
     110.08,8.79,15.68,1.,1.,7.3,8.79,10.14,1.,1.,5.57,15*1.,8.79,
     24*1.,5.98,9*1./
      DATA B/9.18,9.32,10.82,11.15,11.92,15.79,9.13,9.69,13.85,
     18.47,9.32,11.70,0.,0.,6.57,9.32,9.13,0.,0.,8.04,15*0.,9.32,
     24*0.,6.82,9*0./
      DATA C/1.88,1.51,1.36,0.85,1.39,0.47,1.38,1.35,2.31,
     11.16,1.51,-0.27,0.,0.,0.66,0.51,1.38,0.,0.,-0,15*0.,1.51,
     24*0.,1.61,9*0./
      DATA XP /19.04,19.62,23.72,24.87,28.94,31.33,20.65,22.04,30.82,
     119.71,19.62,27.11,0.,0.,14.53,18.73,20.65,0.,0.,13.61,15*0.,
...
C      CHARACTER*14 FMT
     ...
     1NAA,ZBB,IIN,IUT,IPLO,IPU,IGEO,TITLE(39),ISTOP,ISEQ,ISER,DHFM,
     2INFORM,IPDB,IESP,IHB,IRES,IARC
      PARAMETER(IX=201,IY=IX,IX3=IX*3,IX4=IX*4,IX6=IX*6)
      COMMON /GEOM1/ X(3,IX),ISORT(IX),A(3,IX),NRA(4,3,IX),IRST(4,IX),
     1    IRZ,ARST(IX),NUMM(IX),AB(3,IX),IFIX(IX),IMOL(IX),IAT(IX)
      COMMON /GEOM3/ VV(IX,IX),VS(IX,IX),EP,SWI(IX),NN,P(IY,IY),CS(IX),
     1QSIG(IX),IBOND,NBOND(2,IX4),RBON(IX4),IANG,NANG(3,IX6),RANG(IX6),
...
C
   ...
   30 Z(K1)=Q(3,K)
     ...
     3IDELT,NDELT(4,IX6),RDELT(IX6),QSEFF(IX),QADD(IX),NEPS,ctors(ix6)
      common /dyna/ idz,idd(4,40),irdz(ix6),idyn,dystar
      common /elnegs/ xneg(ix,ix),xeta(ix,ix)
      dimension wpi(50,50),pif(50),xpi(50),xsig(50),epi(50),esig(50),
     1piw(ix,ix),slone(50)
      data pif/3.,2.,3.,2.,3.,2.,3.,0.92,1.0,41*3./
      data xpi/5.87,5.87,7.32,7.32,9.63,9.63,7.44,9.35,12.202,
     112.202,40*10.0/
      data xsig/14.98,14.98,20.68,20.68,27.19,27.19,17.02,20.54,33.91,
     133.91,9*10.00,7.176,30*20./
      data epi/5.545,5.545,6.50,6.50,7.665,7.665,5.06,5.698,8.786,
     18.786,40*6.0/
      data esig/6.15,6.15,6.82,6.82,8.10,8.10,5.55,6.225,9.615,
     19.615,9*8.00,6.422,30*8.00/
      do 1 i=1,naa
      do 1 j=1,naa
      xneg(i,j)=0.
      xeta(i,j)=0.
...
c      wpi(6,3)=1.151
      ...
      do 40 j=1,naa
      is2=isort(j)+1
      if (vs(i,j).gt.0.)sumpi=sumpi+wpi(is1,is2)
   40 continue
      do 50 j=1,naa
      if (vs(i,j).gt.0.) then
      is2=isort(j)+1
      ...
      piw(i,j)=wpi(is1,is2)/sumpi*pif(is1)
      end if
   50 continue
      do 60 j=1,naa
      if (vs(i,j).gt.0.) then
      xneg(i,j)=piw(i,j)*xpi(is1)+(1.-piw(i,j))*xsig(is1)
      xeta(i,j)=piw(i,j)*epi(is1)+(1.-piw(i,j))*esig(is1)
      end if
   60 continue
   30 continue
      write (6,1001)
 1001 format (/,'Elneg. and hardness of AO(i) in bond to j')
      write (6,1003)
 1003 format ('    i    j     elng      hardn    %p')
      do 1000 i=1,naa
      do 1000 j=1,naa
 1000 if (xneg(i,j).gt.0.) write (6,1002) i,j, xneg(i,j),xeta(i,j),
     1piw(i,j)
 1002 format (2i5,3f10.3)
      return
      end
      SUBROUTINE SETGEO
C
...
c Kelling
      ...
      do 2001 j=1,naa
      if (vs(i,j).gt.0.)then
      if (isort(j).eq.19) nsub(i)=nsub(i)+1
      end if
 2001 continue
      do 2002 i=1,naa
      if (nsub(i).gt.4) nsub(i)=1
 2002 continue
c      do 2003 i=1,naa
c      write (6,*) i, nsub(i)
c 2003 continue
...
C
      ...
      DO 324 I=1,NAA
      DO 325 J=I+1,NAA
c      IF (VS(I,J).NE.0..AND.VS(I,J).NE.-4.) GOTO 325
...
cs
     ...
     1 rvdw(50),ion(50)
      common /elnegs/ xneg(ix,ix),xeta(ix,ix)
c Kelling
      DIMENSION XE(IX),QK(IX),qsa(ix),qsz(ix,ix),qsza(ix,ix)
C
...
C
 ...
 12   continue
c
c     calculate bond based partial charges
c
          DO 20 I=1,NAA
          do 21 j=i,naa
c          dqik=0.33333*cs(i)
c          dqjl=0.33333*cs(j)
      ...
      if (vs(i,j).gt.0.) then
          do 22 k=1,naa
          if (k.ne.j.and.vs(i,k).gt.0.) dqik=dqik+qsza(i,k)
 22   continue
          do 23 l=1,naa
          if (l.ne.i.and.vs(j,l).gt.0.) dqjl=dqjl+qsza(j,l)
 23   continue
          xeli=xneg(i,j)+dqik*xeta(i,j)/3.
          xelj=xneg(j,i)+dqjl*xeta(j,i)/3.
          dqij=abs((xeli-xelj)/(xeta(i,j)+xeta(j,i)))
          if (abs(abs(dqij)-abs(qsza(i,j))).gt.0.00002) iconv=1
          if (xeli.gt.xelj) then
          qsz(i,j)=-dqij
          qsz(j,i)=dqij
          else
          qsz(i,j)=dqij
...
c
      PARAMETER(IX=201,IY=IX*2/3,IX4=IX*4,IX6=IX*6)
